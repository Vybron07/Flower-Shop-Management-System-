...
class StaffFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(top, text="Staff", font=("Arial", 22, "bold")).pack(side="left")
        ...
        for j, h in enumerate(headers):
            ctk.CTkLabel(hf, text=h, font=("Arial",12,"bold"),
                         text_color="gray").grid(row=0, column=j, padx=10, pady=4, sticky="w")
    ...
    def refresh(self):
        ...
        if not staff_list:
            ctk.CTkLabel(self.scroll, text="No staff found.", text_color="gray").pack(pady=30)
            return
    ...
    def open_add_dialog(self):
        ...
        role = ctk.CTkOptionMenu(dialog, values=["Librarian","Admin","Assistant"], width=320)
        role.pack(padx=20)
        def save():
            ...
            dialog.destroy()
            self.refresh()
        ctk.CTkButton(dialog, text="Add Staff", width=320, command=save).pack(pady=20)